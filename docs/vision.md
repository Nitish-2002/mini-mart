...
Decisions:
  ...
  - decision-15 :: End User requires an account (phone + OTP) | kind: decision | summary: "Checkout requires a phone-number + OTP account, not guest checkout, so order history, reordering, and address reuse are possible from day one." | spec: [Vision Decisions](vision.md#vision-decisions) | alternatives: "Guest checkout only — simplest to build, but re-collects phone/address every order and rules out order history." | reversal_trigger: "OTP delivery/verification proves a real conversion blocker at checkout after launch."
...
OpenQuestions:
  - oq-12 :: Account model for End User | kind: openquestion | summary: "Does an End User need to create an account, or is guest checkout with just a phone number enough for Phase 1? — resolved, see decision-15." | spec: [Vision Decisions](vision.md#vision-decisions)
...
- **Account-based checkout** (phone + OTP, decision-15) with a tiered delivery fee — minimum order to check out, free delivery above a threshold, a smaller fee below it (decision-04, stage 00) — and tax-inclusive pricing (decision-19).
...
1. **End User requires a phone + OTP account, not guest checkout** ([decision-15](#)) — enables order history and reordering from day one. Revisit if OTP verification proves a conversion blocker.
